...
const ShopWithSidebar = () => {
  ...
  const [categories, setCategories] = useState<{ name: string; id: number; products: number; isRefined: boolean }[]>([]);
      ...
      try {
        ...
        setCategories(
          catData.filter((c) => c.isActive).map((cat: Category) => ({
            ...
            isRefined: selectedCategoryId === cat.id,
          }))
        );
  ...
  useEffect(() => {
    fetchProducts();
  }, [debouncedSearchQuery, selectedCategoryId, minPrice, maxPrice, currentPage, sortBy, selectedAttributes]);
  ...
  useEffect(() => {
    ...
    if (catId) setSelectedCategoryId(parseInt(catId));
  }, [searchParams]);
