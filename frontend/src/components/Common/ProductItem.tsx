...
const ProductItem = ({ item }: { item: Product }) => {
  ...
  const handleAddToCart = async (e?: React.MouseEvent) => {
    ...
    // Fix: If variants are missing in the list view, fetch them first
    if (!productToAdd.variants || productToAdd.variants.length === 0) {
        try {
            const data = await ProductService.getById(productToAdd.id);
            const fullProduct = mapToProduct(data);
            if (fullProduct.variants && fullProduct.variants.length > 0) {
                productToAdd = fullProduct;
            }
        } catch (error) {
            console.error("Failed to fetch product details for Add to Cart:", error);
        }
    }
    ...
    if (productToAdd.variants && productToAdd.variants.length > 0) {
        productToAdd = {
            ...productToAdd,
            id: productToAdd.variants[0].id,
            price: productToAdd.variants[0].price || productToAdd.price,
            discountedPrice: productToAdd.variants[0].price || productToAdd.discountedPrice
        };
    }
  ...
  return (
    <div className="group relative">
      ...
      {showWishlistToast && (
        <div className="absolute top-2 left-1/2 -translate-x-1/2 z-50 animate-slideDown">
          <div className="bg-green-500 text-white px-4 py-2 rounded-lg shadow-lg flex items-center gap-2 whitespace-nowrap">
            ...
            <span>Đã thêm vào yêu thích! ({wishlistItems.length + 1} sản phẩm)</span>
          </div>
        </div>
      )}
      ...
      <div className="relative overflow-hidden rounded-lg bg-[#F6F7FB] aspect-square w-full mb-4 flex items-center justify-center p-4">
        <Link href={`/shop-details?id=${item.id}`} className="relative w-full h-full flex items-center justify-center">
          <Image 
